...
pub fn split_whitespace_into_tuple(input: &str) -> Result<(String, String)> {
    let mut bounds = input.split_whitespace();
    let lower_bound = bounds.next().context("Unable to unwrap first value")?;
    let upper_bound = bounds.next().context("Unable to unwrap second value")?;

    Ok((lower_bound.to_owned(), upper_bound.to_owned()))
}
